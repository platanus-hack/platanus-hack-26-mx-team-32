...
import { scrapeAndSeedFacebookPatterns, geocodeMissingLocations } from "../lib/facebook-scraper.js";

async function displayExtractedPatterns() {
  ...
  const { data, error } = await supabase
    ...
    .select("tone_description, tone_keywords, location_text, location_region, location_latitude, location_longitude, post_date, image_descriptions, post_url")
  ...
  console.log("\n╔══════════════════════════════════════════════════════════════╗");
  console.log(`║  EXTRACTED PATTERNS  (${data.length} most recent records)`.padEnd(64) + "║");
  console.log("╚══════════════════════════════════════════════════════════════╝");
  ...
  data.forEach((row, i) => {
    ...
    const coords  = row.location_latitude != null
...
async function main() {
  ...
  // Retroactively geocode any older rows that have location_text but no coordinates
  ...
  await displayExtractedPatterns();
